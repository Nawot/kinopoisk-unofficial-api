...
class TestGetFacts(aiounittest.AsyncTestCase):
    ...
    async def test_is_list(self):
        for i in (301, 178707, 859908):
            response = await self.client.get_facts(i)
            self.assertIsInstance(response, list)


    async def test_contain_facts(self):
        for i in(301, 178707, 859908):
            response = await self.client.get_facts(i)
            for fact in response:
                self.assertIsInstance(fact, (Fact, Blooper))
